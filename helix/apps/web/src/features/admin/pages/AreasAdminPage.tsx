...
import { AreaAdminRow } from '@helix/shared';
...
import {
  ...
  useDeactivateArea,
  useActivateArea,
} from '../hooks/useAdminAreasMutations';
import {
  RefDataEntryModal,
  FormValues,
  FieldDescriptor,
...
export function AreasAdminPage(): JSX.Element {
  const [modalOpen, { open: openModal, close: closeModal }] = useDisclosure(false);
  const [editTarget, setEditTarget] = useState<AreaAdminRow | null>(null);
  ...
  const { data: rows, isPending, isError, refetch } = useAdminAreas();
  const { mutate: create, isPending: creating } = useCreateArea();
  const { mutate: update, isPending: updating } = useUpdateArea();
  const { mutate: deactivate, isPending: deactivating } = useDeactivateArea();
  const { mutate: activate, isPending: activating } = useActivateArea();
  ...
  function handleOpenEdit(row: AreaAdminRow) {
    setEditTarget(row);
    setModalApiError(null);
    openModal();
  }
  ...
  function handleDeactivate(row: AreaAdminRow) {
  ...
  return (
    <PageLayout>
      <Stack gap="md">
      <PageHeader title="Areas" icon={<IconLayoutGrid size={22} />} actions={<Button color="stadaRed" onClick={handleOpenAdd}>Add Entry</Button>} />
      ...
      {isError && (
        <Alert color="stadaRed" title="Failed to load areas">
